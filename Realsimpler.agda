...
open import Altrat as ℚ using (ℚ; -_ ; _*_; _÷_; _≤_; *≤*; ≃⇒≡; _-_; _+_)
...
data _==_ : (x : ℝ) -> (y : ℝ) -> Set where
 _*==*_ : {n : ℕ}(x : ℝ) -> (y : ℝ) -> (∣ ℝ.f x n - ℝ.f y n ∣ ≤  (+ 1 ÷ (suc n))  {fromWitness (λ {i} → 1-coprimeTo (suc n))} ℚ.+ (+ 1 ÷ (suc n))  {fromWitness (λ {i} → 1-coprimeTo (suc n))}) -> x == y
